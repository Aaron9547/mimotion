...
phone_regex = re.compile(r'^(13\d|14[5|7]|15\d|18\d|19\d)\d{8}$')
...
class ZeppLife:
    ...
    def _get_access_code(self, account, password):
        """ 获取 Access Code """
        url = f'https://api-user.huami.com/registrations/{account}/tokens'
        data = {
            'client_id': 'HuaMi',
            'password': password,
    ...
    def _sync_data(self, app_token, user_id, step):
        """ 同步数据 """
    ...
    def start(self, account: str, password: str, step: int = None):
        """ 启动刷步 """
        ...
        result = self._sync_data(app_token, user_id, step)
        return result
